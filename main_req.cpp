...
int main() {
    ...
    vector<int> request = {10, 20, 30, 40};
    ...
    for(auto req : request) {
        ...
        for(int a = 0; a < avg; a++) {
            Graph graph(req, 15, 10000);
            int round = 12;
            vector<ll> result(round);
            #pragma omp parallel for
            for(int r = 0; r < round; r++) {
                EA myalgo(graph);
                result[r] = myalgo.one_plus_one(10000);
            }
            ...
            for(int r = 0; r < round2; r++) {
                EA2 myalgo2(graph);
                result2[r] = myalgo2.one_plus_one(10000);
            }
        ...
        cout << req << " " << (ll)my_res / avg << " " << (ll)my_res2 / avg<< " " << (ll)greedy_res / avg << endl;
    }
}
